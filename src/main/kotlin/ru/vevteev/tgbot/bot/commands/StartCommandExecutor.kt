...
@Component
class StartCommandExecutor(private val messageSource: MessageSource) : CommandExecutor {
    ...
    override fun commandDescription(locale: Locale): String = messageSource.getMessage("command.description.start", locale)

    override fun perform(update: Update, bot: TelegramLongPollingBot, arguments: List<String>) {
        update.run {
            bot.execute(createSendMessage(messageSource.getMessage("msg.start", arrayOf(firstName()), locale(arguments))))
